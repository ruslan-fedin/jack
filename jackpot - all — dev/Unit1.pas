...
Try
 ...
 key:=IdHTTP1.Get('http://rusla2sq.beget.tech/lic/'+IntToStr(SerialNum)+'.lic');

 if EncodeLic(key, false) = IntToStr(SerialNum) then
   begin
       label1.Caption:='Лицензия зарегистрирована. Спасибо)';
 ...
 try
  ...
  CloseFile(f);
  showmessage('Файл с лицензией для '+edit1.text+' создан успешно');
 except
 showmessage('Ошибка');
 end;
 end;
         ...
         begin
           showmessage('Нет лицензии !');
         end;
Except
 showmessage('Нет лицензии!');
 label1.Caption:='Нет лицензии';
